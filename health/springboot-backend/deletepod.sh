...
# deletepod.sh - Find and delete healthai-backend-springboot pod(s) in the health namespace
...
echo ""
echo "🔄 Checking remaining healthai-backend-springboot pods..."
kubectl get pods -n health | grep "healthai-backend-springboot" || echo "✅ No healthai-backend-springboot pods remaining"
...
echo ""
echo "⏳ Waiting for new healthai-backend-springboot pod to be created..."
sleep 5

# Wait for new pod to be created and get its name
NEW_POD=""
for i in {1..30}; do
    NEW_POD=$(kubectl get pods -n health --no-headers | grep "healthai-backend-springboot" | grep "Running\|ContainerCreating" | head -1 | awk '{print $1}')
    if [ ! -z "$NEW_POD" ]; then
        echo "✅ Found new healthai-backend-springboot pod: $NEW_POD"
        break
    fi
...
if [ ! -z "$NEW_POD" ]; then
    ...
    echo "❌ No new healthai-backend-springboot pod found after waiting"
    echo "📊 Current pods in health namespace:"
    kubectl get pods -n health
fi
