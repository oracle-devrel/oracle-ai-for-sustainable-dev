...
class OrderServiceEventProducer {
    ...
    String updateDataAndSendEvent(
            DataSource dataSource, String orderid, String itemid, String deliverylocation, Span activeSpan,
            String spanIdForECID) throws Exception {
        ...
        try {
            ...
            System.out.println("updateDataAndSendEvent jdbcConnection:" + jdbcConnection + " activeSpan:" + activeSpan
                    + " about to insertOrderViaSODA...");
            Order insertedOrder = insertOrderViaSODA(orderid, itemid, deliverylocation, jdbcConnection);
            if (OrderResource.crashAfterInsert)
                System.exit(-1);
            ...
            TracingMessageProducer producer = new TracingMessageProducer(session.createPublisher(topic),
                    orderResource.getTracer());
            ...
            // publisher.publish(topic, objmsg, DeliveryMode.PERSISTENT,2,
            // AQjmsConstants.EXPIRATION_NEVER);
            session.commit();
            System.out.println(
                    "updateDataAndSendEvent committed JSON order in database and sent message in the same tx with payload:"
                            + jsonString);
    ...
    void updateOrderViaSODA(Order order, Connection jdbcConnection) throws SQLException {
